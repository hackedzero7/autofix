...
export default function Dashboard({ user }: { user: { name: string } }) {
  ...
  return <main className="mx-auto max-w-6xl space-y-8 px-4 py-10">
    <div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-sm text-primary">Administrator</p><h1 className="mt-1 text-3xl font-bold">Welcome, {user.name}</h1><p className="mt-2 text-muted-foreground">Manage the admin blog library. Blogs stay inside this CRM.</p></div><Button variant="outline" disabled={busy} onClick={() => run(async () => { await api("logout", "POST"); window.location.assign("/crm/login") })}>Sign out</Button></div>

  ...
  return <main className="mx-auto max-w-6xl space-y-8 px-4 py-10">
    ...
    <section className={panel}><div className="flex flex-wrap items-center justify-between gap-4"><h2 className="text-xl font-semibold">Your blogs</h2><div className="flex flex-wrap gap-2"><Button variant="destructive" disabled={loading || busy || !posts.length} onClick={() => {
      ...
      void run(async () => {
        ...
        setPosts([]); setEditor(null); setSections(""); setKeywords(""); setSearch("")
        setMessage(`Deleted ${result.deletedCount} blogs.`)
        await reload()
      })
    }}>Delete all blogs</Button><Button variant="outline" disabled={!posts.length || busy} onClick={() => download(posts.map(p => blogInput.parse(p)), "blogs.json")}>Export JSON</Button><Button disabled={busy} onClick={() => edit(blank())}>New blog</Button></div></div><Input className="my-5" aria-label="Search blogs" placeholder="Search title, category, or status…" value={search} onChange={e => setSearch(e.target.value)} />

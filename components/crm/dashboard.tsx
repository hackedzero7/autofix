  ...
  return <main className="mx-auto max-w-6xl space-y-8 px-4 py-10">
    ...
    <section className={panel}><div className="flex flex-wrap items-center justify-between gap-4"><h2 className="text-xl font-semibold">Your blogs</h2><div className="flex flex-wrap gap-2"><Button variant="destructive" disabled={loading || busy || !posts.length} onClick={() => {
      ...
      void run(async () => {
        ...
        setPosts([]); setSelectedIds([]); setEditor(null); setSections(""); setKeywords(""); setSearch("")
        setMessage(`Deleted ${result.deletedCount} blogs.`)
        await reload()
      })
    }}>Delete all blogs</Button><Button variant="outline" disabled={!posts.length || busy} onClick={() => download(posts.map(p => blogInput.parse(p)), "blogs.json")}>Export JSON</Button><Button disabled={busy} onClick={() => edit(blank())}>New blog</Button></div></div><Input className="my-5" aria-label="Search blogs" placeholder="Search title, category, or status…" value={search} disabled={busy} onChange={e => { setSearch(e.target.value); setSelectedIds([]) }} />
      <div className="mb-5 flex flex-wrap items-center gap-3 rounded-lg border border-border bg-background p-4">
        <label className="flex cursor-pointer items-center gap-2 text-sm">
          <input type="checkbox" className="h-4 w-4 accent-red-600" aria-label={search ? "Select all matching blogs" : "Select all blogs"} ref={element => { if (element) element.indeterminate = selectedIds.length > 0 && !allSelected }} checked={allSelected} disabled={loading || busy || !visibleIds.length} onChange={e => setSelectedIds(e.target.checked ? visibleIds : [])} />
          {search ? "Select all matching blogs" : "Select all blogs"}
        </label>
